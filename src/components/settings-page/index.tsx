...
export function SettingsPage() {
	const [view, setView] = useState<'networks' | 'members'>('networks');
	const changeTabCallback = (tab: 'networks' | 'members') => {
		setView(tab);
	};
	return (
		<>
				...
				<Container className="max-w-6xl mx-auto pt-6 pb-4">
					...
					<div className="grid w-full items-start gap-6 md:grid-cols-[180px_1fr] lg:grid-cols-[250px_1fr]">
						...
						>
							<Link
								href="#"
								className={`text-primary ${view === 'networks' ? 'font-semibold' : ''}`}
								onClick={() => setView('networks')}
							>
								Custom networks
							</Link>
							{isMonitoringFeatureActive() && (
								<Link
									href="#"
									className={`text-primary ${view === 'members' ? 'font-semibold' : ''}`}
